...
$("#mainDiv").droppable({
    drop: function (ev, ui) {
        ...
        if (!ui.draggable.hasClass('foo') && ui.draggable.hasClass('tower')) {
            ...
            var Class = ui.draggable.attr("class");
            title = ui.draggable.text().trim();
            var item = $('<table class="foo elementTable device_list_name' + (diagramLenght + 1) + ' ' + Class + ' name="' + title + '" id="' + (diagramLenght + 1) + '" style="left: ' + droppedLeft + 'px; top: ' + droppedTop + 'px;"><tr class="tableBody tableHeader"  id="tower_' + (diagramLenght + 1) + '"><th class="thClass" id="towe-remove"><span style="margin-left:22px;" class="header' + (diagramLenght + 1) + '">' + title + (diagramLenght + 1) + '</span></span><span class="minimized" id="minimized"><img src="/image/arrow_drop_down_white.png" width="17px" height="17px" alt="Settings" title="Minimize" id="minimizedImg' + (diagramLenght + 1) + '"/></span><span class="paly_stop_tower" id="paly_stop_tower"><img src="/Icons/play.png" width="17px" height="17px" alt="Settings" title="Play/Stop" id="paly_stop_tower' + (diagramLenght + 1) + '"/></span><span class="GpsSetting" id="GpsSetting"><img src="/Icons/pignon.png" width="17px" height="17px" alt="Settings" title="Settings" /></th></tr><tr><td class="add' + (diagramLenght + 1) + '"><ul id="deviceAdd' + (diagramLenght + 1) + '" name="dropDivs"class="ui-droppable"></ul></td></tr></table>');
            $(this).append(item);
            diagramLenght++;
            $('#settingsDiv').html("");
            foo();
            ...
            drop: function (ev, ui) {
                ...
                if (!ui.draggable.hasClass('foo') && !ui.draggable.hasClass('display') && ~con.indexOf("jtk-connected")) {
                    ...
                    if (!ui.draggable.hasClass('foo') && ui.draggable.hasClass('device_list')) {
                        ...
                        droppedTop = height;
                        var item = $('<table class="foo elementTable tableBodyTower tower_name' + (id + 1) + '" ' + Class + '" name="' + title + '" id="' + (id + 1) + '" title="tower_' + idTarget + '" style="left: ' + 3 + 'px; top: ' + droppedTop + 'px;min-width: 245px;"><tr class="tableBodyTowers tableHeader" style="display:inline-table" id="towerDevice' + (idTarget) + '_' + (id + 1) + '"><th class="thClass" id="device-remove"><span value="' + title + '" class="device_header' + (id + 1) + '">' + titleName + '</span><span class="paly_stop_device" id="paly_stop_device"><img src="/Icons/play.png" width="17px" height="17px" alt="Settings" title="Play/Stop" id="paly_stop_device' + (diagramLenght + 1) + '"/></span><span class="device_settings"><img src="/Icons/pignon.png" width="17px" height="17px" alt="Settings" title="Settings"data-toggle="modal" data-target="#myModal" /></span></th></tr></table>');
