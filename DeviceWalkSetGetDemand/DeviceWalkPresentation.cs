...
namespace AdminPanelDevice.DeviceWalkSetGetDemand
{
    public class DeviceWalkPresentation
    {
        ...
        public void PointConnections(Array[] connections)
        {
            deviceWalkBusinessLogic.CreatePointConnections(connections);
        }

        public List<DeviceType> DeviceGroupList(int devcieGroup)
        {
            return deviceWalkBusinessLogic.DeviceGroupListReturn(devcieGroup);
        }
        public WalkMibSetting LoadWalkMibSetting (string deviceName, string towerName, int deviceID, int defineWalk, List<WalkTowerDevice> walkList) {
            return deviceWalkBusinessLogic.LoadWalkMibSettingInformation(deviceName, towerName, deviceID, defineWalk, walkList);
        }
    }
}